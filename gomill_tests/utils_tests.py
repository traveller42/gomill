...
def test_sanitise_utf8(tc):
    ...
    tc.assertEqual(su(b""), b"")
    tc.assertEqual(su(u""), b"")
    tc.assertEqual(su(b"hello world"), b"hello world")
    tc.assertEqual(su(u"hello world"), b"hello world")
    s = u"test \N{POUND SIGN}"
    b = s.encode("utf-8")
    tc.assertIs(su(b), b)
    tc.assertEqual(su(s), b)
    tc.assertEqual(su(u"test \N{POUND SIGN}".encode("latin1")), b"test ?")
